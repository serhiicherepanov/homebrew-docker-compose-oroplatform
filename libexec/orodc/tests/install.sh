...
# Install test dependencies with composer in test-cli container
run_with_spinner "Installing test dependencies" \
  "${TEST_COMPOSE_CMD} run --rm test-cli composer install --prefer-dist --no-progress --no-interaction"

msg_ok "Test environment ready"
